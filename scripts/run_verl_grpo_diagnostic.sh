#!/usr/bin/env bash
# DEV / DIAGNOSTIC ONLY: one-step Qwen3-0.6B GRPO parameter-update check.

set -euo pipefail

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TRAIN_FILE="${PROJECT_ROOT}/data/grpo_smoke/diagnostic.parquet"
...
CHECKPOINT_DIR="${CHECKPOINT_DIR:-${PROJECT_ROOT}/outputs/verl_grpo_sql_diagnostic}"
ROLLOUT_DATA_DIR="${ROLLOUT_DATA_DIR:-${PROJECT_ROOT}/outputs/verl_grpo_sql_diagnostic_rollouts}"

export VLLM_USE_FLASHINFER_SAMPLER=0

for required_file in "${TRAIN_FILE}" "${VAL_FILE}" "${DATABASE_FILE}" "${REWARD_FILE}"; do
    if [[ ! -f "${required_file}" ]]; then
        echo "Missing diagnostic prerequisite: ${required_file}" >&2
...
exec python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name=sql_grpo_nonthinking_diagnostic_v1 \
